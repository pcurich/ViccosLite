...
using System.Text;
using ViccosLite.Core.Domain.Settings.Seo;
using ViccosLite.Core.Infrastructure;

namespace ViccosLite.Services.Setting.Seo
{
    public static class SeoExtensions
    {
        ...
        public static string GetSeName(string name, bool convertNonWesternChars, bool allowUnicodeCharsInUrls)
        {
            if (String.IsNullOrEmpty(name))
                return name;
            const string OK_CHARS = "abcdefghijklmSoftqrstuvwxyz1234567890 _-";
            ...
            foreach (char c in name.ToCharArray())
            {
                ...
                {
                    if (char.IsLetterOrDigit(c) || OK_CHARS.Contains(c2))
                        sb.Append(c2);
                }
                else if (OK_CHARS.Contains(c2))
                {
                    sb.Append(c2);
                }
            }
