...
namespace ViccosLite.Services.Tasks
{
    ...
    {
        private const int NOT_RUN_TASKS_INTERVAL = 60*30; //30 minutes
        ...
        public void Initialize()
        {
            ...
            var notRunTasks = scheduleTasks
                .Where(x => x.Seconds >= NOT_RUN_TASKS_INTERVAL)
                .Where(
                    x =>
                        !x.LastStartUtc.HasValue ||
                        x.LastStartUtc.Value.AddSeconds(NOT_RUN_TASKS_INTERVAL) < DateTime.UtcNow)
                .ToList();
